...
- fix: bound the OpenSL ES buffer-queue drain on Android. A stalled OpenSL callback queue could otherwise block `ma_device_stop()` indefinitely, including the automatic idle-stop path. The drain now has a 200–1000 ms polling budget based on the configured queue duration before stop and queue clearing continue #333
