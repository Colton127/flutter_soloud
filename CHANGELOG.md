...
- fix: engine initialization, Dart callback registration and teardown are now scoped to an initialization lease taken by the owning `FlutterEngine`. Calling `prepareEngineInit()` first does not win the race to the native init mutex, so without it a superseded initialization could build over a newer engine, a superseded isolate could overwrite the live engine's callbacks, and a queued teardown could dispose a live engine or strip its ownership. An initialization that loses the claim while the audio device is opening now disposes what it built instead of leaving an orphaned device running
