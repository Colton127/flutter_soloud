...
- add `setAudioDeviceIdleTimeout(Duration?)` to control how long the audio output device keeps running after the engine goes idle before it is paused (defaults to 500 ms on every platform); pass `null` to keep it running indefinitely (a device-level replacement for playing a silent looping sound to keep a backgrounded audio app alive)
