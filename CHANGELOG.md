...
- fix: bound the OpenSL ES buffer-queue drain in the vendored miniaudio (Android below API 26). Upstream spins until the queue reports empty with no timeout, so a stalled OpenSL callback thread made `ma_device_stop()` — and therefore the idle-stop — hang forever #333. The wait is now capped at the real queue depth, so the wakelock is still released on every Android version
...
- Android: the native library is loaded on first plugin attach rather than from a static initializer, so it is no longer force-loaded on the main thread at app start for apps that never play audio, and a load failure can no longer crash plugin registration
