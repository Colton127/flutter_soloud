...
Merges the device-lifecycle work that had accumulated on `main` on top of 4.1.7. Everything below is new relative to 4.1.7; the fixes that reached upstream separately (voice-ended dispatch, playback error propagation, off-UI-thread `init()`/`deinitAsync()`, `changeDevice()` correctness, the FlutterEngine lifecycle work) are already described in the entries below and are unchanged.
...
- fix: the automatic audio device start/stop (triggered when a sound is played or all sounds are paused, e.g. on iOS or Android) no longer blocks the UI thread. `play()`, `play3d()`, `setPause()` and `pauseSwitch()` stay synchronous, but the blocking native device start now runs on the same background scheduler thread that already handles the deferred device stop. Thanks to @Colton127
  - **behaviour change**: because the automatic start is now asynchronous, `play()`, `play3d()`, `setPause()` and `pauseSwitch()` no longer throw `SoLoudAudioDeviceFailedToStartCppException` — they cannot observe the outcome of a start that has not happened yet. Voice-allocation failures still throw `SoLoudFailedToStartPlaybackCppException`, and the explicit `startAudioDevice()` still reports device-start failures. `playClocked()`, `play3dClocked()`, `playScheduled()` and `textToSpeech()` start the device synchronously (they need a running clock) and are unchanged.
...
- fix: `changeDevice()` is now asynchronous (`Future<void>`) and runs device enumeration and replacement off the UI isolate, which could otherwise ANR on Android. The replacement device is only started when the old one was running or the idle policy requires it, so a device stopped by `stopAudioDevice()` or by the idle timeout stays stopped across the swap. Thanks to @Colton127
