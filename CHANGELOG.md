...
- add `setAudioDeviceIdleTimeout()` to configure how long the audio output device keeps running while the engine is idle before it is stopped, on every platform: a `null` timeout keeps it running indefinitely (a device-level replacement for playing a silent looping sound to keep a backgrounded audio app alive), `Duration.zero` stops it as soon as possible, and a positive timeout sets the grace period (default 500 ms); the timeout is also applied right after `init()`
