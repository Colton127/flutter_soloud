...
- fix: a data race on `mPostClipScaler`. `init()` set it after `soloud.init()` had already started the device, so the mixer was reading it from the audio thread while the calling thread wrote it. ThreadSanitizer flags it in `clip_internal()`; the store now takes the audio mutex like every other setter used during playback. Thanks to @Colton127
