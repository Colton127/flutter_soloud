...
- Android (API 26+, AAudio) now stops the audio device when idle (no active voices) like every other platform, releasing the audioserver `AudioMix` partial wakelock #250; use `setAudioDeviceIdleTimeout()` to keep it running. Android below API 26 uses the OpenSL ES backend, whose device stop drains its buffer queue in an unbounded loop and can hang if the callback thread stalls (#333), so the automatic idle-stop stays suppressed there
